...
// Function for View Bill Summary
void viewStats() {
...
void placeOrder() {
    ...
    for (int i = 0; i < orderCount; i++) {
        fprintf(order, "%d %s %.2f %d %.2f\n", orderItem[i].id, orderItem[i].name, orderItem[i].price, orderItem[i].quantity, orderItem[i].total);
    }
...
// Function For Customer and Revenue update
...
void makePayment() {
    ...
    if (total == amount)
        printf("Thanks for your payment. Have a Good day\n");
    else if (total > amount)
        printf("Please pay the rest: %.2f\n", total - amount);
    else
...
void customerMenu() {
    ...
    do {
        ...
        switch (choice) {
            ...
            case 6: printf("Thank you! Visit again.\n"); break;
            default: printf("Invalid choice. Try again.\n");
        }
    } while (choice != 6);
}
